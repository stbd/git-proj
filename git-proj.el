...
(defun git-proj-goto-other-impl (search-root match-rules filename)
  ...
  (if (> (length split-name) 1)
      ...
      (let (
            ...
            (default-directory search-root) ; Set run directory for call-process
            )
        (setq result-buffer "")    ; Buffer to show multiple matches
...
(defun git-proj-goto-other ()
  ...
  (if (not (equal (buffer-file-name) nil))
      (let ((
             filename (car (last (split-string (buffer-file-name) "/")))
            ))
